...
function setupDepartments(){
  ...
  grid.querySelectorAll(".dept-item").forEach(btn => {
    btn.addEventListener("click", () => {
      const dept = btn.dataset.dept;

      // Prefer an exact category match if the catalog already has one;
      // otherwise fall back to a text search across name + category
      // so the button still works before categories are renamed to match.
      const cats = getCategories();
      if(Object.prototype.hasOwnProperty.call(cats, dept)){
        activeFilter = dept;
        searchTerm = "";
      } else {
        activeFilter = "all";
